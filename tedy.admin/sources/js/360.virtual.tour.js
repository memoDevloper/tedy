...
$(document).ready(function() {
    ...
    function onloadPage() {
        var photosId = $('#photosId').val();
        $.get('/ajax/get-360-single/' + photosId, function(data) {
            var dataHash = 1;
            $.each(data, function(index, photo){
                $('.owl-carousel').append(
                    '<div data-hash="' + dataHash + '">' +
                        '<li data-pan-id="' + dataHash + '" data-pan-desc="' + photo.desc + '" id="pan-menu-' + dataHash + '" class="pan-menu">' +
                            '<a class="pan-menu-item"  id="pan-image-' + dataHash + '" data-pan-href="' + photo.url + '" href="#' + dataHash + '">' +
                                '<img class="pan-slide-thumb" src="' + photo.thumbnail_url + '">' +
                            '</a>' +
                        '</li>' +
                    '</div>');
                ++dataHash;
